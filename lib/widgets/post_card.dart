  ...
  class _PostCardState extends State<PostCard> {
    ...
    void initState() {
      ...
      if (widget.previewAudio != null) {
        ...
        audioFileToPlay = Audio(clientAppAudioFilePath: post?.audioLink!, title: ''); // Assume other required fields are filled as needed
      }
      _checkLikeStatus();
    }
    ...
    void _loadUserData() {
      ...
      widget.post?.profilePictureUrl = userDataProvider.currentUser?.profilePictureUrl;
    }
    ...
    void _checkLikeStatus() async {
      ...
      bool isLiked = await apiLikePost.checkLikeStatus(widget.post!.postId.toString());
      setState(() {
    ...
    Future<void> loadComposition(String compositionId) async {
      ...
      Composition? loadedComposition = await ApiCompositionGet.fetchComposition(compositionId);
    ...
    Widget build(BuildContext context) {
      ...
      final ImageProvider<Object> defaultImageProvider = AssetImage('assets/background_sky.jpg');
                  ...
                  children: [
                    GestureDetector(
                      onTap: () {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => ProfilePage(userId: post?.userId)),
                        );
                      },
                      child: CircleAvatar(
                        backgroundImage: post!.profilePictureUrl != null && post.profilePictureUrl!.isNotEmpty
...
/*                            boxShadow: [
                        ...
                        padding: EdgeInsets.all(8.0), // Adjust the padding to fit your design
                        child: Icon(
                          Icons.fullscreen_rounded, // Toggles between play icon and account tree icon
                          color: Colors.white,
                          size: 40,
                        ),
                      ),
                    ...
                    children: [
                      InkWell(
                        ...
                        child: Container(
                          ...
                          padding: EdgeInsets.all(8.0), // Adjust the padding to fit your design
                          child: Icon(
                            toggle_composition_tree ? Icons.play_arrow : Icons.account_tree, // Toggles between play icon and account tree icon
                            color: Colors.deepPurple,
                            size: 40,
                          ),
                        ),
                ...
                child: Opacity(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(post?.ratingAverage.toString() ?? '', style: TextStyle(color: Colors.white),textAlign: TextAlign.center),
                      SizedBox(height: 8, width: 80,),
                      IconButton(
                        padding: EdgeInsets.all(0.0),
                        icon: Icon(
                          // Use the null-coalescing operator to ensure a non-nullable condition
                          widget.post?.isLikedByUser ?? false ? Icons.favorite : Icons.favorite_border,
                          color: Colors.white,
                          size: 40,
                        ),
                        onPressed: () async {
                          if (widget.post != null) {
                            ApiLikePost apiLikePost = getIt<ApiLikePost>();
                            try {
                              await apiLikePost.likePost(widget.post!.postId.toString());
                              // Toggle the isLikedByUser state and update the like count accordingly
                              setState(() {
                                widget.post!.isLikedByUser = !(widget.post!.isLikedByUser ?? false);
                                if (widget.post!.isLikedByUser ?? false) {
                                  widget.post!.totalLikes = (widget.post!.totalLikes ?? 0) + 1;
                                } else {
                                  widget.post!.totalLikes = (widget.post!.totalLikes ?? 0) - 1;
                                }
                              });
    ...
    Future<void> showCommentsDialog(BuildContext context, String postId) async {
      ...
      showModalBottomSheet(
        ...
        builder: (BuildContext context) {
          ...
          final double availableHeight = screenHeight - keyboardPadding.bottom - 100;
          ...
          return Padding(
            ...
            child: Container(
              child: Column(
                ...
                children: [
                  Container(
                    height: availableHeight * 0.9, // Allocate 70% of available height to comments list
                  ...
                  Padding(
                    ...
                    child: TextField(
                      controller: commentController, // Attach the TextEditingController
                        ...
                        suffixIcon: IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (commentController.text.isNotEmpty) {
                              ...
                              showCommentsDialog(context, postId); // Optionally refresh the comments
                            }
                            commentController.clear(); // Clear the text field after posting
                          },
                        ),
                      ),
                    ),
